...
**Обычный путь с 17.08 — панель:** `my.vshage.app` → «Веб-регистрация» →
«Создать». Экран задаёт всё, что описано ниже, и сразу показывает обе ссылки.
Панель ходит в этот же admin-API через прокси в organizer-api
(`/admin/webreg/*`), который держит `WEBREG_ADMIN_TOKEN` на своей стороне.
Ручной конфиг ниже остаётся рабочим и нужен, когда панель недоступна или
правку удобнее сделать батчем.
...
```jsonc
{
  ...
  "venue": { … },                          // из шага 1
  "form": {                                // ЧТО спрашиваем и что обязательно
    "name":        {"enabled": true,  "required": true},
    "full_name":   {"enabled": false, "required": false},  // ФИО как в документе
    "email":       {"enabled": true,  "required": true},   // несёт билет и напоминания
    "phone":       {"enabled": false, "required": false},
    "tg":          {"enabled": true,  "required": false},
    "affiliation": {"enabled": false, "required": false},
    "pass_note": "Нужно для пропуска в здание"             // видно под полем ФИО
  },
  "ticket_mode": "qr",                     // qr | code | off
  "publish_afisha": true,                  // карточка на afisha.vshage.app
  "publish_vshage": true,                  // кнопка «Открыть во Вшаге»
...
Пропуск `form` целиком означает «дай разумное по умолчанию»: имя и почта
обязательны, телеграм спрашиваем необязательно. Событие, заведённое **до**
миграции 004, читается со старой формой (имя + телеграм + вуз, всё
обязательно) и без билетов — живое меро не должно менять форму под людьми,
которые её в этот момент заполняют.

### 4. Ссылки

| Кому | Ссылка |
|---|---|
| В канал / в личку | `https://vshage.app/e/shag` |
| Витрина в афише | `https://afisha.vshage.app/shag` |
| Организатору | `https://vshage.app/e/shag/manage?key=<manage_key>` |
| Билет гостя | `https://vshage.app/e/shag/t/<code>` |
| Выгрузка | кнопка «Скачать CSV» на той же странице |

Ссылка регистрации и ссылка в афише — **разные страницы by design**
(директива 17.08). В афише обложка и полное описание, это витрина; на
`/e/<slug>` обложки нет и описание сжато до первого абзаца — туда приходят
записаться, а не читать. Из афиши кнопка «ЗАРЕГИСТРИРОВАТЬСЯ» ведёт на
`/e/<slug>`, обратно — кнопка «Открыть в афише».

Отметка на входе: организатор открывает ссылку билета **со своим ключом** —
`…/t/<code>?key=<manage_key>` — и жмёт «Отметить пришедшим». Проще всего
отсканировать QR гостя телефоном и дописать `?key=…` один раз: браузер
подставит его в следующий раз из истории. Повторное сканирование время
прихода не перезаписывает.
...
## Замер запаса прочности (DEV, 2026-08-17)

Мерялось на DEV-хосте локально, чтобы не мерить свой интернет вместо сервиса.

| Что | Результат |
|---|---|
| Рендер страницы на сервере | **5 мс** (API события — 1 мс) |
| 600 открытий страницы, 60 одновременно | 600×200, за 2.37 с ⇒ **≈253 стр/с**, p95 = 307 мс |
| 300 регистраций, 30 одновременно | 299×201 + 1×200, за 0.65 с ⇒ **≈460 записей/с**, в базе ровно 300 |
| Повтор тех же 300 в ВЕРХНЕМ регистре | 300×200 «уже записан», в базе по-прежнему 300 |
| Строк `WEBREG_SUBMIT` в журнале | 301 — на каждый сабмит, включая дублирующий |

То есть анонс на несколько тысяч человек, которые приходят в течение минут,
проходит с большим запасом: узкое место — не сервис. Полторы секунды, которые
видно с ноутбука, — это TLS-рукопожатие до хоста, у главной афиши ровно такое же.

Предсказание до замера было «сотни в секунду на чтение, десятки на запись»;
запись оказалась на порядок быстрее ожидаемого, потому что вставка одна и без
сопутствующих чтений. Ограничение прибора: мерялось с самого хоста, без TLS и
без реальной мобильной сети, — это потолок сервиса, а не то, что почувствует
человек в метро.
