...
function loadFaces(): Promise<Face[]> {
  if (!facesPromise) {
    facesPromise = Promise.all([
      ...
      // Кэшировать ОТКЛОНЁННЫЙ промис нельзя: одна неудача чтения файла
      // выключила бы карточки до конца жизни процесса.
      .catch((e) => {
        facesPromise = null;
        throw e;
      });
...
function placeLine(ev: Record<string, unknown>): string {
  ...
  for (const p of parts) {
    if (!out.some((s) => s.toLowerCase().includes(p.toLowerCase())))
      out.push(p);
  }
  if (!out.length && typeof ev.online_url === "string" && ev.online_url)
    return "Онлайн";
  return out.join(", ");
}
...
export const GET: RequestHandler = async ({ params, fetch, url }) => {
  ...
  const res = await fetch(
    `${backend}/api/events/${encodeURIComponent(params.id)}`,
  );
      ...
      div(
        {
          ...
          height: 630,
          backgroundImage:
            "linear-gradient(90deg, rgba(10,10,10,0) 0%, rgba(10,10,10,0.55) 100%)",
        },
        "",
      ),
    );
